...
# Flask app
doa_unknown_user = Flask(__name__)
...
doa_unknown_user.permanent_session_lifetime = timedelta(minutes=30)
...
# Route for doa_unknown_user requests
@doa_unknown_user.route('/get_response_unreg', methods=['POST'])
def get_response_unreg():
    if flask_request.is_json:
        user_message = flask_request.get_json().get('message')
        if not user_message:
            return jsonify({'error': 'No message provided'}), 400
        response, _ = get_answer(faq_data, conversation_log, user_message, intent_probabilities)
        return jsonify({'response': response})
    return jsonify({'error': 'Invalid JSON input'}), 400

# Start the Flask app
if __name__ == "__main__":
    doa_unknown_user.run(debug=True, host='0.0.0.0', port=5003)